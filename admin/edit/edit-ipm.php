  ...
  <body>
    ...
    <div class="row no-gutters mt-5">
      ...
      <?php
        ...
        $id = $_GET["id"];
        $query = mysqli_query($koneksi, "SELECT * FROM ipm WHERE id_ipm='$id' ");
        $result = mysqli_fetch_assoc($query);

      ?>

      <div class="col-md-9 p-5 pt-2">
      <h3><i class="fas fa-tv mr-2"></i>EDIT IPM</h3>
      <hr>

      <form method="post" enctype="multipart/form-data">
          <div class="form-group">
              <img src="../../asset/img/ipm/<?= $result['foto'] ?>" width="100"> <br>
              <label for="exampleFormControlFile1">Pilih Foto</label>
              <input type="file" name="foto" class="form-control-file" id="exampleFormControlFile1">
            </div>
            <div class="form-group">
              <label for="exampleFormControlInput1">ipm Yang Di Raih</label>
            ...
            <button class="btn btn-warning"><a href="../ipm.php" class="text-white" style="text-decoration: none;">Kembali</a></button>
    ...
    <?php   
          ...
          if (isset($_POST['submit'])) {
            ...
            $dir = '../../asset/img/ipm/'. $foto;

            if (!$foto) {
              

              $query = "UPDATE ipm SET judul ='$judul', deskripsi ='$deskripsi' WHERE id_ipm='$id' ";
              $result = mysqli_query($koneksi, $query);

              if (!$result) {
                echo "<script>
                    alert('gagal mengubah ipm');
                    window.location.assign('edit-ipm.php');
                </script>";
              } else {
                echo "<script>
                    alert('berhasil mengubah ipm');
              ...
              $query = "UPDATE ipm SET foto ='$foto', judul ='$judul', deskripsi ='$deskripsi' WHERE id_prestasi='$id' ";
              $result = mysqli_query($koneksi, $query);
