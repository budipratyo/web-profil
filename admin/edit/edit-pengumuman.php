...
<html lang="en">
  ...
  <body>
    ...
    <div class="row no-gutters mt-5">
      ...
      <h3><i class="fas fa-tv mr-2"></i>EDIT PENGUMUMAN</h3>
      <hr>

      <form method="post" enctype="multipart/form-data">
          <div class="form-group">
              <label for="exampleFormControlInput1">Judul Pengumuman</label>
              <input type="text" name="judul_pengumuman" value="<?= $result['judul_pengumuman']; ?>" class="form-control" id="exampleFormControlInput1">
            </div>
            <div class="form-group">
              <label for="exampleFormControlInput1">Tanggal Upload</label>
              <input type="date" name="tanggal" value="<?= $result['tanggal']; ?>" class="form-control" id="exampleFormControlInput1">
            </div>
            <div class="form-group">
              <label for="exampleFormControlTextarea1">Deskripsi</label>
              <textarea class="form-control" name="deskripsi" id="exampleFormControlTextarea1" rows="3"><?= $result['deskripsi']; ?></textarea></div>
            <button class="btn btn-warning"><a href="../Pengumuman.php" class="text-white" style="text-decoration: none;">Kembali</a></button>
    ...
    <?php   
          ...
          if (isset($_POST['submit'])) {
            ...
            $deskripsi = $_POST['deskripsi'];  
              ...
              $query = "UPDATE pengumuman SET judul_pengumuman ='$judul_pengumuman', tanggal = '$tanggal', deskripsi ='$deskripsi' WHERE id_pengumuman='$id' ";
              $result = mysqli_query($koneksi, $query);

              if (!$result) {
                echo "<script>
                    alert('gagal mengubah pengumuman');
                    window.location.assign('edit-pengumuman.php');
                </script>";
              } else {
                echo "<script>
