  ...
  <body>
    ...
    <div class="row no-gutters mt-5">
      ...
      <?php
        ...
        $id = $_GET["id"];
        $query = mysqli_query($koneksi, "SELECT * FROM ekstra WHERE id_ekstra='$id' ");
        $result = mysqli_fetch_assoc($query);

      ?>

      <div class="col-md-9 p-5 pt-2">
      <h3><i class="fas fa-user-edit mr-2"></i>EDIT EKSTRAKULIKULER</h3>
      <hr>

      <form method="post" enctype="multipart/form-data">
            <div class="form-group">
              <label for="exampleFormControlInput1">Nama Ekstra</label>
              <input type="text" name="ekstra" value="<?= $result['nama_ekstra']?>" class="form-control" id="exampleFormControlInput1">
            </div>
    ...
    <?php   
          ...
          if (isset($_POST['submit'])) {
            ...
            $ekstra = $_POST['ekstra'];
              ...
              $query = "UPDATE ekstra SET nama_ekstra ='$ekstra' WHERE id_ekstra='$id' ";
              $result = mysqli_query($koneksi, $query);

              if (!$result) {
                echo "<script>
                    alert('gagal mengubah ekstra');
                    window.location.assign('edit-ekstra.php');
                </script>";
              } else {
                echo "<script>
                    alert('berhasil mengubah ekstra');
