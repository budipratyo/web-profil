  ...
  <body>
    ...
    <div class="row no-gutters mt-5">
      ...
      <?php
        ...
        $id = $_GET["id"];
        $query = mysqli_query($koneksi, "SELECT * FROM misi WHERE id_misi='$id'");
        $result = mysqli_fetch_assoc($query);

      ?>

      <div class="col-md-9 p-5 pt-2">
      <h3><i class="fas fa-tv mr-2"></i>EDIT misi</h3>
      <hr>

      <form method="post" enctype="multipart/form-data">
            <div class="form-group">
              <label for="exampleFormControlInput1">Judul misi</label>
              <input type="text" name="misi" value="<?= $result['misi']?>" class="form-control" id="exampleFormControlInput1">
            </div>
            <button class="btn btn-warning"><a href="../misi.php" class="text-white" style="text-decoration: none;">Kembali</a></button>
    ...
    <?php   
          ...
          if (isset($_POST['submit'])) {
            ...
            $misi = $_POST['misi'];
              ...
              $query = "UPDATE misi SET misi ='$misi' WHERE id_misi='$id' ";
              $result = mysqli_query($koneksi, $query);

              if (!$result) {
                echo "<script>
                    alert('gagal mengubah misi');
                    window.location.assign('edit-misi.php');
                </script>";
              } else {
                echo "<script>
                    alert('berhasil mengubah misi');
