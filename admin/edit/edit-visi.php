  ...
  <body>
    ...
    <div class="row no-gutters mt-5">
      ...
      <?php
        ...
        $id = $_GET["id"];
        $query = mysqli_query($koneksi, "SELECT * FROM visi WHERE id_visi='$id' ");
        $result = mysqli_fetch_assoc($query);

      ?>

      <div class="col-md-9 p-5 pt-2">
      <h3><i class="fas fa-tv mr-2"></i>EDIT VISI</h3>
      <hr>

      <form method="post" enctype="multipart/form-data">
            <div class="form-group">
              <label for="exampleFormControlInput1">Judul Visi</label>
              <textarea name="visi" class="form-control" rows="10"><?= $result['visi']?></textarea>
            </div>
            <button class="btn btn-warning"><a href="../visi.php" class="text-white" style="text-decoration: none;">Kembali</a></button>
    ...
    <?php   
          ...
          if (isset($_POST['submit'])) {
            ...
            $visi = $_POST['visi'];
              ...
              $query = "UPDATE visi SET visi ='$visi' WHERE id_visi='$id' ";
              $result = mysqli_query($koneksi, $query);

              if (!$result) {
                echo "<script>
                    alert('gagal mengubah visi');
                    window.location.assign('edit-visi.php');
                </script>";
              } else {
                echo "<script>
                    alert('berhasil mengubah visi');
