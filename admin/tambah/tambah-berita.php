...
<html lang="en">
  ...
  <body>
    ...
    <div class="row no-gutters mt-5">
      ...
      <h3><i class="fas fa-tv mr-2"></i>Tambah Berita</h3>
      <hr>
      ...
      <form method="post" enctype="multipart/form-data">
          ...
          if (isset($_POST['submit'])) {
           ...
           if (move_uploaded_file($_FILES['foto']['tmp_name'], $dir)) {
             $query = "INSERT INTO berita(foto, judul, deskripsi) VALUES ('$foto', '$judul', '$deskripsi')";
             $result = mysqli_query($koneksi, $query);

             if (!$result) {
               echo "<script>
                   alert('gagal menambah berita');
                   window.location.assign('tambah-berita.php');
               </script>";
             } else {
               echo "<script>
