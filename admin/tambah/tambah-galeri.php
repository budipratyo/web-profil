...
<html lang="en">
  ...
  <body>
    ...
    <div class="row no-gutters mt-5">
      ...
      <h3><i class="fas fa-photo-video mr-2"></i>Tambah Galeri</h3>
      <hr>
      ...
      <form method="post" enctype="multipart/form-data">
          ...
          if (isset($_POST['submit'])) {
           ...
           if (move_uploaded_file($_FILES['foto']['tmp_name'], $dir)) {
             $query = "INSERT INTO galeri(foto, deskripsi) VALUES ('$foto', '$deskripsi')";
             $result = mysqli_query($koneksi, $query);

             if (!$result) {
               echo "<script>
                   alert('gagal menambah galeri');
                   window.location.assign('tambah-galeri.php');
               </script>";
             } else {
               echo "<script>
                   alert('berhasil menambahkan galeri');
