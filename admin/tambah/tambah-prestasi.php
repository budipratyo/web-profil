...
<html lang="en">
  ...
  <body>
    ...
    <div class="row no-gutters mt-5">
      ...
      <h3><i class="fas fa-trophy mr-2"></i>Tambah Prestasi</h3>
      <hr>
      ...
      <form method="post" enctype="multipart/form-data">
          ...
          if (isset($_POST['submit'])) {
           ...
           if (move_uploaded_file($_FILES['foto']['tmp_name'], $dir)) {
             $query = "INSERT INTO prestasi(foto, judul, deskripsi) VALUES ('$foto', '$judul', '$deskripsi')";
             $result = mysqli_query($koneksi, $query);

             if (!$result) {
               echo "<script>
                   alert('gagal menambah presatsi');
                   window.location.assign('tambah-prestasi.php');
               </script>";
             } else {
               echo "<script>
