    ...
    <div class="row no-gutters mt-5">
      <div class="cold-md-2 bg-dark mt-2 pr-3 pt-4">
        <ul class="nav flex-column ml-3 mb-5">
          ...
          <li class="nav-item">
            <a class="nav-link text-white" href="ipm.php"><i class="fas fa-users mr-2"></i> IPM</a><hr class="bg-secondary">
          </li>
          <li class="nav-item">
      ...
      <h3><i class="fas fa-users mr-2"></i>IPM</h3>
      <hr>
      
      <a href="tambah/tambah-ipm.php" class="btn btn-primary"><i class="fas fa-plus mr-2"></i>Tambah IPM</a>
      ...
      <?php
        ...
        $no = 1;
        $query = mysqli_query($koneksi, "SELECT * FROM ipm");
        while ($result = mysqli_fetch_assoc($query)) {
      ?>
          <tbody class="text-center">
            <tr>
              <th scope="row"><?= $no++ ?></th>
              <td><img src="../asset/img/ipm/<?= $result['foto'] ?>" width="100"></td>
              <td><?= $result['judul'] ?></td>
              <td><?= $result['deskripsi'] ?></td>
              <td>
                <a href="edit/edit-ipm.php?id=<?= $result['id_ipm']; ?>" class="btn btn-success">Edit</a>
              </td>
              <td>
                <a href="hapus/hapus-ipm.php?id=<?= $result['id_ipm']; ?>" class="btn btn-danger">Hapus</a>
              </td>
            </tr>
          </tbody>
      <?php  } ?>
